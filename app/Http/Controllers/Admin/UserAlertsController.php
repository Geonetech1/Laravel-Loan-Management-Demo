...
class UserAlertsController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = UserAlert::with(['users', 'expiring_contract'])->select(sprintf('%s.*', (new UserAlert)->table));
            ...
            $table->addColumn('expiring_contract_expires_on', function ($row) {
                return $row->expiring_contract ? $row->expiring_contract->expires_on : '';
            });

            $table->rawColumns(['actions', 'placeholder', 'user', 'expiring_contract']);
    ...
    public function create()
    {
        ...
        $expiring_contracts = Contract::all()->pluck('expires_on', 'id')->prepend(trans('global.pleaseSelect'), '');

        return view('admin.userAlerts.create', compact('users', 'expiring_contracts'));
    }
    ...
    public function show(UserAlert $userAlert)
    {
        ...
        $userAlert->load('users', 'expiring_contract');
